...
__host__ Kluster* kmeansCycle_SOA(Points soaPoints, int numPoints,Point* selectedCentroids, int numClusters, float maxSSE, bool printConsole) {
    ...
    /*size_t arraySizeBytes = numPoints * sizeof(double) * 3;
    Points d_soaPoints;
    CUDA_CHECK_ERROR(cudaMalloc((void**)&d_soaPoints.x, arraySizeBytes));
    d_soaPoints.y = d_soaPoints.x + numPoints;
    d_soaPoints.z = d_soaPoints.y + numPoints;
    CUDA_CHECK_ERROR(cudaMemcpy(d_soaPoints.x, soaPoints.x, arraySizeBytes * 3, cudaMemcpyHostToDevice));*/
